...
exports.getCourses = async (req, res) => {
  ...
  console.log("departmentId id at backend is->", departmentId);
  ...
  try {
    ...
    console.log("fetched courses at backend is->", sql);
    res.json(sql);
  } catch (error) {
...
exports.getActiveEnrollments = async (req, res) => {
  try {
    const [sql] = await db.execute(
      "select distinct count(student_id) as active_enrollments from enrollments where course_id is not null"
    );
    res.json(sql);
