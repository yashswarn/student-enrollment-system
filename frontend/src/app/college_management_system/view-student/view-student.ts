...
@Component({
  ...
  standalone:true,
  imports: [CommonModule,RouterModule, FormsModule,MatMenuModule,MatButtonModule],
...
export class ViewStudent implements OnInit {
  ...
  onSearch(){
    this.isSearch=true;
    this.studentService.getSearchedName(this.currentPage,this.limit,this.searchedName).subscribe((data:any)=>{
      if(data){
